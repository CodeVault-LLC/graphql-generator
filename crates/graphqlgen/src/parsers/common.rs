        ...
        if tokens.get(*index) == Some(&Token::ParenOpen) {
            *index += 1;
            ...
            while *index < tokens.len() && tokens.get(*index) != Some(&Token::ParenClose) {
                ...
                let value_type = match tokens.get(*index) {
                    Some(Token::String(_)) => TypeRef::Named("String".to_string()),
                    Some(Token::Name(n)) => TypeRef::Named(n.clone()),
                    other => return Err(anyhow!("Unexpected directive value type: {:?}", other)),
                };
                *index += 1;

                args.push(InputValue { name, value_type });
